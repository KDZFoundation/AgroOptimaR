...
export const Card: React.FC<CardProps> = ({ title, icon, badge, footer, children, className, variant = 'default' }) => {
    const variants = {
    ...
    return (
        <div className={cn('rounded-3xl overflow-hidden card-shadow', variants[variant], className)}>
            ...
            {footer && (
                <div className="px-6 py-4 bg-gray-50/50 border-t border-gray-100 italic text-sm text-text-secondary">{footer}</div>
            )}
        </div>
    )
}
