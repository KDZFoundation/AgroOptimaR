...
const PRAKTYKI = [
    ...
    { kod: 'E_WSG', nazwa: 'Wymieszanie słomy z glebą', punkty: 1, opis: 'Rozdrobnienie i wymieszanie słomy z glebą po zbiorze plonu głównego.' },
]

export default function RolnictwoWeglowePage() {
    const [selectedPraktics, setSelectedPraktics] = useState<string[]>([])
    const [powierzchnia, setPowierzchnia] = useState(214.43)

    const totalPoints = selectedPraktics.reduce((acc, kod) => {
        const p = PRAKTYKI.find(item => item.kod === kod)
        return acc + (p ? p.punkty : 0)
    }, 0)
    ...
    const togglePraktyka = (kod: string) => {
        setSelectedPraktics(prev =>
    ...
    return (
        <div className="space-y-8">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-3xl font-bold text-text-primary">Rolnictwo Węglowe</h2>
                    <p className="text-text-secondary mt-1">Zarządzaj praktykami i symuluj punkty dla swojego gospodarstwa.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <Card className="flex flex-col justify-center">
                    <div className="flex justify-between items-center mb-2">
                        <span className="text-sm font-semibold text-text-secondary">Próg wejścia (1.25x)</span>
                        <Tooltip content="Minimalna liczba punktów wymagana dla Twojej powierzchni UR.">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {PRAKTYKI.map((p) => (
                            <div
                                key={p.kod}
                                onClick={() => togglePraktyka(p.kod)}
                                className={`p-5 rounded-2xl border transition-all cursor-pointer group relative overflow-hidden ${selectedPraktics.includes(p.kod)
                                        ? 'border-green-600 bg-green-50/50 shadow-md'
                                        : 'border-border-custom bg-white hover:border-green-500/50'
                                    }`}
                            >
                                <div className="flex justify-between items-start mb-3">
                                    <div className={`p-2 rounded-xl transition-colors ${selectedPraktics.includes(p.kod) ? 'bg-green-600 text-white' : 'bg-gray-100 text-text-muted group-hover:bg-green-100 group-hover:text-green-700'}`}>
                                        <Leaf className="w-5 h-5" />
                                    </div>
                                    <Badge variant={selectedPraktics.includes(p.kod) ? 'success' : 'neutral'}>
                                        +{p.punkty} PKT
                                    </Badge>
                                </div>
                                ...
                                {selectedPraktics.includes(p.kod) && (
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="p-4 bg-green-50 rounded-2xl border border-green-100 flex items-start gap-3">
                                ...
                                <p className="text-xs text-text-secondary leading-relaxed">
                                    Pamiętaj, że praktyki <b>'{PRAKTYKI[6].nazwa}'</b> i <b>'{PRAKTYKI[4].nazwa}'</b> mają specjalne wymogi dotyczące terminów realizacji.
                                    Sprawdź sekcję wymogów przed deklaracją.
                                </p>
                            </div>
                    ...
                    <div className="p-6 bg-green-900 rounded-3xl text-white relative overflow-hidden">
                        <div className="relative z-10">
                            ...
                            <p className="text-xs text-green-100/80 leading-relaxed mb-4">
                                Dodanie praktyki 'Plan nawożenia' zwiększy Twoje dopłaty o ok. 2 400 PLN i pomoże w spełnieniu normy GAEC 7.
                            </p>
